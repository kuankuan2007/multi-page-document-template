...
compressing.zip.uncompress(zipPath, uncompressDirPath).then(()=>{
    ...
    fonts.forEach((font)=>{
        fs.copyFileSync(path.join(filePath, "font", font), path.join(process.cwd(), "src","fontello", "font", font));
        console.log(`Copy ${font} success`);
    });
    fs.copyFileSync(path.join(filePath, "config.json"), path.join(process.cwd(), "src","fontello", "config.json"));
